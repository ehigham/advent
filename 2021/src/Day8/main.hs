...
part1 :: [Entry] -> IO ()
part1 entries = printf "Occurences of 1, 4, 7 or 8 output values = %d.\n"
        $ length (entries >>= filter ((`S.member` digits) . length) . output)
  where
    digits = S.fromList [2,3,4,7] :: HashSet Int
...
-- values?
--
-- | Return that set of digits that have the specified number of segments
part2 :: a -> IO ()
part2 = const (fail "Not implemented.")
...
main = do
    ...
    let entries = map read (lines contents) :: [Entry]
    putStr "Part 1: " >> part1 entries
    putStr "Part 2: " >> part2 entries