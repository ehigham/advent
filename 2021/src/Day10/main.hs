...
part1 :: [String] -> IO ()
part1 = printf "Syntax error score = %d.\n" . score
  where
...
part2 :: [String] -> IO ()
part2 = printf "Autocomplete score = %d.\n" . winningScore . rights . map score
  where
    winningScore :: [Integer] -> Integer
    winningScore = liftA2 (!!) sort ((`quot` 2 ) . length)

    score :: String -> Either ParseError Integer
    score x = parseChunks x <&> fst <&> stackPop <&> go 0
      where
...
main = do
    [input] <- Env.getArgs
    program <- readFile input <&> lines
    putStr "Part 1: " >> part1 program
    putStr "Part 2: " >> part2 program
