...
import Day2.Dive ( Command ( .. ), Horizontal, Depth )
...
interpret :: Command -> (Horizontal, Depth) -> (Horizontal, Depth)
interpret (Forward x) (h, d) = (h + x, d)
interpret (Up      x) (h, d) = (h, d - x)
interpret (Down    x) (h, d) = (h, d + x)
...
main = do
    ...
    let commands            = map read (lines contents) :: [Command]
        (horizontal, depth) = foldl (flip interpret) (0, 0) commands
    printf "Position:\n\thorizontal = %d\n\tdepth = %d\n\tproduct = %d\n"
        horizontal
        depth
        (horizontal * depth)
