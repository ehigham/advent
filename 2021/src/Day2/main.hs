...
part1 :: [Command] -> IO ()
part1 commands =
    let (horizontal, depth) = foldl interpret (0, 0) commands in
        printf "Horizontal position * depth = %d.\n" (horizontal * depth)
  where
    interpret (h, d) (Forward x) = (h + x, d)
    interpret (h, d) (Up      x) = (h, d - x)
    interpret (h, d) (Down    x) = (h, d + x)
...
part2 :: [Command] -> IO ()
part2 commands =
     let (horizontal, depth, _) = foldl interpret (0, 0, 0) commands in
        printf "Horizontal position * depth = %d.\n" (horizontal * depth)
  where
    interpret (h, d, a) (Forward x) = (h + x, d + a * x, a    )
    interpret (h, d, a) (Up      x) = (h    , d        , a - x)
    interpret (h, d, a) (Down    x) = (h    , d        , a + x)
...
main = do
    ...
    putStr "Part 1: " >> part1 commands
    putStr "Part 2: " >> part2 commands
