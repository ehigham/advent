...
import Day2.Dive ( Command ( .. ), Horizontal, Depth, Aim )
...
-- depth?
interpret :: Command -> (Horizontal, Depth, Aim) -> (Horizontal, Depth, Aim)
interpret (Forward x) (h, d, a) = (h + x, d + a * x, a    )
interpret (Up      x) (h, d, a) = (h    , d        , a - x)
interpret (Down    x) (h, d, a) = (h    , d        , a + x)
...
main = do
    ...
    let commands                 = map read (lines contents) :: [Command]
        (horizontal, depth, aim) = foldl (flip interpret) (0, 0, 0) commands
    printf "Position:\n\thorizontal = %d\n\tdepth = %d\n\taim = %d\n\tproduct = %d\n"
        horizontal
        depth
        aim
