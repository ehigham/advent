...
-- marker is detected?
part1 :: Text -> IO ()
part1 = printf "First marker after character %d\n"
      . findMarker 4
...
-- marker is detected?
part2 :: Text -> IO ()
part2 = printf "Start-of-message marker after character %d\n"
      . findMarker 14


findMarker :: Int -> Text -> Int
findMarker len text = runST $ do
    idx <- newSTRef (len - 1)

    whileM $ do
        i <- readSTRef idx
        modifySTRef' idx (+1)
        let window = S.fromList [T.index text k | k <- [i - len + 1 .. i]]
        return $ S.length window < len
