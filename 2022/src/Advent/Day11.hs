...
-- docus on the two most active monkeys if you want any hope of getting your
...
inputParser = V.fromList . sortOn index <$> monkey `sepEndBy1` spaces
  where
    ...
    operation :: Parser (WorryLevel -> WorryLevel)
    operation = do
...
newtype WorryLevel = Level { unLevel :: Integer }
  deriving newtype (Show, Read, Eq, Num)


data Monkey' s = Monkey
    { index :: Int
    , items :: s
