...
import Advent.Share.ParsecUtils  (int, pairwise, parseFile)

desc :: String
desc = "Day 15: Beacon Exclusion Zone"
...
-- frequency, which can be found by multiplying its x coordinate by 4000000 an
...
inputParser = pairwise (,) sensor (string ": ") beacon `sepEndBy` newline
  where
    sensor = string "Sensor at " *> point
    beacon = string "closest beacon is at " *> point
    point = pairwise (,) (string "x=" *> int) (string ", ") (string "y=" *> int)
