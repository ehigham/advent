...
part1 :: [[Char]] -> IO ()
part1 = printf "Sum of priorities of common item types = %d\n"
      . sum
      . map ( sum
            . map priority
            . S.toList
...
part2 :: [[Char]] -> IO ()
part2 = printf "Sum of priorities of item badges of each 3-elf group = %d\n"
      . sum
      . concatMap (map priority . S.toList . foldl1 S.intersection . map S.fromList)
      . chunksOf 3
...
inputParser = many1 letter `sepEndBy1` (void newline <|> eof)
