...
part1 = printf "Sum of signal strengths = %d\n"
      . sum
      . (\xs -> map (strength xs) [20, 60, 100, 140, 180, 220])
      . runMachine
  where
    strength xs cycle' = cycle' * (xs !! pred cycle')
...
runMachine :: [Instruction] -> [Int]
runMachine instrs = snd $ execRWS (traverse f instrs) undefined 1
  where
    f Noop = output
    ...
    output = get >>= tell . pure


runDisplay :: [Int] -> [String]
runDisplay = chunksOf width . zipWith (curry draw) [i `mod` width | i <- [0..]]
  where
    draw (pos, sprite) =
        if abs (sprite - pos) > 1
            then dark
...
inputParser = instr `sepEndBy` spaces
  where
    instr = choice
