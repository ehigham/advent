...
part1 = printf "Number of points snake tail visited (length 2) = %d\n"
      . S.size
      . S.fromList
      . simulate 2
...
-- hoose how to arch your body to avoid being hit. Fortunately, your simulation
...
part2 :: [(Direction, Int)] -> IO ()
part2 = printf "Number of points snake tail visited (length 10) = %d\n"
      . S.size
      . S.fromList
      . simulate 10


simulate :: Int                 -- snake length
         -> [(Direction, Int)]  -- list of moves
         -> [Point]             -- points traversed by snake
simulate len steps = snd $ evalRWS (traverse eval steps) undefined start
  where
    start = ((0, 0), replicate (len - 1) (0, 0))

    eval :: (Direction, Int) -> RWS a [Point] (Point, [Point]) ()
    eval (d, n) = replicateM_ n $ do
        (h, ts) <- get
        let h' = move d h
            ts' = tail $ scanl chase h' ts
        put (h', ts')
        tell [last ts']
